...
The checks exist because of the seven. Working at that pace is what turned a
slow mistake into a fast one, and it is why the answer had to be mechanical
...
<details>
<summary>`oracle.py` is the project, and `columns.py` decides in advance what any difference it finds is allowed to mean</summary>
...
`src/twicerun/columns.py` runs before any row is read, because what it decides is the join key. Integers, strings, dates and `DECIMAL` cannot come back different from a correct query run twice, so a difference in one of those is a finding, while `FLOAT` and `DOUBLE` can, because a parallel reduction adds in whatever order the threads finish in, so a difference there is a measurement. `DECIMAL` on the exact side is the fix this tool recommends for the float aggregate bug rather than an accident of classification: DuckDB's decimal sum is fixed-point and reassociates exactly. Nested types are the third case and they are refused rather than compared, because casting a `STRUCT` to `VARCHAR` makes one holding 1.0 differ from one holding 1.00 and counting elements ignores the elements, so a type a later DuckDB release adds stops the run instead of falling through to whichever branch came last.

`src/twicerun/oracle.py` is the comparison and it is the project. Rows are matched on the exact columns, which turns an assignment problem into a join. `row_number() OVER (PARTITION BY key ORDER BY values)` on both sides preserves multiplicity, so a duplicated row reads as `MULTIPLICITY` rather than as a row count that happens to differ. Every float pair that moved is reported with both its ULP distance and its relative difference, and neither decides anything alone. `src/twicerun/compare.py` is the comparison the oracle replaced, and it is kept because the number it produces is the argument for the replacement: bit-exact multiset equality counts a reassociated float sum exactly as it counts a wrong answer, and the eval runs it as baseline 1 and publishes that count beside the oracle's. `src/twicerun/sql.py` writes all of that out as DuckDB, including the ULP transform: `DOUBLE` to `BIT` gives the raw IEEE-754 layout, `BIT` to `BIGINT` reads those bits as two's complement, and reflecting the negative half about the minimum makes integer distance equal ulp distance.
...
`src/twicerun/report.py` formats what `twicerun run` prints, and two rules of that format live there: every header repeats the DuckDB version and the thread count, because every number under it is specific to both, and a step that never diverged prints `0 of 4` rather than a word like stable. `src/twicerun/manifest.py` is what records those two conditions next to the artifacts, and its `rows_read` is the term count the reassociation bound in `policy.py` is a function of. That file also says where the count is too small, since anything a step pulls in through `ctx.sql` never reaches it, and where it is too large, since rows brought into scope are not terms behind one output value.
...
`src/twicerun/cli.py` is argument parsing and the map from exception to exit code, which is the only place those codes are assigned. Baseline 4 imports `exit_code` from there rather than deciding for itself what `--no-amplify` would have exited, so the figure it publishes cannot drift away from what the tool returns.
...
- The five ten-trial eval runs A to E, 2026-08-26, and the ten-trial run in a fresh clone that came before the report generator, all in [docs/eval.md](docs/eval.md). None of them wrote JSON out, which is the defect the report generator exists to stop repeating. Run F is the sixth ten-trial run and its JSON is committed, which is why this bullet stops at E, and the fresh-clone run is additional to those six rather than one of them. A and B are the same code and disagree about the headline number anyway, which is the strongest evidence in this repository for its own thesis.
...
- The ten runs behind the premise figure, 2026-08-26: 492, 497, 510, 575, 599, 611, 618, 690, 700 and 738 of the 1,000 group sums differing. All ten are in the premise fold above and its summary line carries the two endpoints. `scripts/measure_duckdb.py` re-derives that measurement in about two seconds, so what no later run reproduces is those ten values rather than the phenomenon behind them. Its ten invariants exit non-zero when one of them breaks, which is the part of that script's output that holds when none of its numbers do.
...
- Around 600 findings on correct float code, 2026-08-26, in [docs/comparison.md](docs/comparison.md) and in `src/twicerun/oracle.py`. It rounds off what bit-exact equality does to `mean_basket`, and the generated version of it is Baseline 1's cell in the table above, a median 610 of the 1,000 rows failing to pair on each side over 10 trials. The strict oracle's own count on that step is a different quantity and appears nowhere as a round number: 618 of 1,000 paired rows moved in the transcript at the top of this file, and the results table carries ulp and relative medians for that step rather than a count.
- Which column the attribution named per step, 2026-08-26, in [docs/comparison.md](docs/comparison.md): `surrogate_id` on both `row_number()` steps and `price_cents` on the merge step, on all 10 passes. The generated attribution line above counts how often leave-one-out named the column the step invented, without naming which column that was in any of them.
